...
class EWC:
    ...
    def setExcitation(self):
        ...
        for i in range(self.tlen):
            T = i * self.tt
            self.dms[self.demand_node, i] = dm * \
                (1. + 0.5 * (0.2 * np.sin(T * 8) - 0.3 * np.cos(5 * T + np.pi / 2) + 0.2 * np.sin(T * 5)**2 - T * 0.1))
        # plot 
    ...
    def transient(self, steady=False): 
        if steady:
            ...
            demand_flow = self.dms[self.demand_node]* np.sqrt(np.abs(self.X[self.M_p +self.demand_node ]- self.z))
            t=np.linspace(0,5,4000)
            plt.figure(1,figsize=(8,3))
